...
let uri =
  "mongodb+srv://admin:admin@cluster0.fpfbz.mongodb.net/myFirstDatabase?retryWrites=true&w=majority";
...
let users = [
  { username: null, color: "firebrick", socketID: null },
  { username: null, color: "darkolivegreen", socketID: null },
  { username: null, color: "gold", socketID: null },
  { username: null, color: "cornflowerblue", socketID: null },
];
...
io.on("connection", (socket) => {
  if (items.length == 0) {
    board(items, size);
  } //if the board is empty we can create a new one else we dont do it so we dont overwrite the board
  socket.on("newUser", (username) => {
    login(users, username, socket); //login adds username and socket id to an object in the users-array, then emits the object
    socket.emit("currentBoard", app.locals.stuff); // this is where we send the board to a user that just connected
  });

  socket.on("updateTile", (update) => {
    // when a user sends that they changed a tile
  ...
  socket.on("disconnect", () => {
    let disconnectedUser = users.find(
